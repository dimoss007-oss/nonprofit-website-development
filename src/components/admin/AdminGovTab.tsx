...
// ─── Контактные лица ────────────────────────────────────────────────────────
function AgencyContacts({ agency }: { agency: Agency }) {
  ...
  const load = () => {
    setLoading(true);
    fetch(`${API}?type=contacts&agency_id=${agency.id}`)
      .then(r => r.json())
    ...
    <div className="border-t border-beige-dark/50 pt-4 mt-4">
      ...
      {loading ? (
        ...
        <div className="space-y-1.5">
          {contacts.map(c => (
            <div key={c.id} className="flex items-center gap-3 p-2.5 rounded-xl border border-beige-dark bg-white group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-0.5 text-xs text-ink/40">
                  ...
                  {c.phone && <span className="flex items-center gap-1"><Icon name="Smartphone" size={10} />{c.phone}</span>}
                </div>
              </div>
              <button onClick={() => archive(c.id)} className="opacity-0 group-hover:opacity-100 p-1 text-ink/30 hover:text-red-500 transition-all flex-shrink-0">
